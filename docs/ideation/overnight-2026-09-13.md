...
|---|---|---|---|
| Instruction Violation | broke an explicit rule it was given | Floor-breach draft caught by the Guard; leak_check under adversarial buyers | none |
| Hallucination | asserts something with no basis in fact | I12: vision's confident item id disagrees with an independent price prior | none |
| Integration Failure | an external step failed and nothing visible handled it | Payment decline → reroute on the ledger; comps timeout → labeled fallback | none |
